...
% Las imágenes se referencian con rutas relativas a Imágenes/
% ============================================
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Justificación económica:} Utiliza componentes accesibles (Arduino UNO, sensores DHT11, BMP280, LDR, LEDs y buzzer), reduciendo el costo total frente a soluciones comerciales equivalentes.
...
\begin{enumerate}[leftmargin=*]
    \item Implementar \textbf{dos entradas digitales} (sensor DHT11 y pulsador de modo) y \textbf{una entrada analógica} (LDR) en el firmware del Arduino. Se contempló originalmente una segunda entrada analógica para ajuste manual de umbral mediante potenciómetro, pero fue sustituida por la propuesta documentada de un actuador LED RGB controlado por comandos desde la PC (no implementado en la versión final).
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Capa de percepción (Hardware):} Constituida por la placa Arduino UNO, sensores (DHT11, BMP280, LDR), actuadores (LEDs, buzzer PWM) y la interfaz USB de comunicación.
...
\begin{figure}[H]
    ...
    \quad LDR (A0) --- A1 (reservado para LED RGB, no implementado)\\[0.5em]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Imágenes/diagrama.png}
...
1 & LED RGB (cátodo común) & \textbf{Planeado / no implementado:} actuador visual cuyo color (verde, amarillo, rojo) indicaría el nivel de temperatura mediante comandos enviados desde la PC. Reemplaza la función originalmente asignada al potenciómetro de umbral. \\
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Entrada analógica reservada (Pin A1):} Inicialmente se planeó conectar un potenciómetro de 10k$\Omega$ para ajuste manual de umbral. Debido a limitaciones de tiempo y priorización del control remoto, esta entrada quedó disponible para futuras expansiones. Como mejora propuesta y documentada, se contempla la implementación de un LED RGB controlado desde la PC mediante comandos seriales, que iluminaría en verde (temperatura normal), amarillo (precaución) o rojo (alerta), proporcionando una indicación visual inmediata sin intervención local.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{LED de alerta (Pin Digital 12):} Se enciende de forma continua cuando alguna variable medida excede el umbral configurado por comando desde la PC, o cuando se activa localmente mediante condición programada.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Imágenes/2.jpeg}
...
\begin{figure}[H]
    ...
    $\downarrow$\\[0.2cm]
    \fbox{Leer pulsador y LDR}\\[0.3cm]
    $\downarrow$\\[0.2cm]
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Módulo de lectura de sensores (readSensors):} Agrupa las funciones de lectura del DHT11 (biblioteca \texttt{DHT.h}), del BMP280 (biblioteca \texttt{Adafruit\_BMP280.h}) y del ADC para el pin A0 (LDR). El pin A1 quedó reservado para la futura integración del LED RGB o potenciómetro de umbral. Convierte los valores brutos del ADC a unidades físicas mediante factores de escala y fórmulas de calibración.
...
// El potenciómetro fue removido del montaje físico.
// El umbral se establece únicamente por comando remoto (SET_TH).
...
\texttt{SET\_TH:xx.x} & Establece el umbral de temperatura a \texttt{xx.x} $^\circ$C, sobrescribiendo el umbral de alerta activo. \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imágenes/3.jpeg}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{Imágenes/monitor.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{Imágenes/landing.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{Imágenes/hardware.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{Imágenes/tinkercad.png}
    \vspace{3cm}}}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Conversión ADC:} Se comprobó que los valores del LDR virtual se mapearan correctamente al rango de luminosidad esperado. El potenciómetro de umbral no fue simulado en la versión final, al quedar documentado como mejora futura (LED RGB controlado desde PC).
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{Imágenes/db.png}
    \vspace{3cm}}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imágenes/1.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imágenes/2.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imágenes/4.jpeg}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Respuesta de actuadores:} El LED de alerta y el buzzer PWM respondieron en menos de 100 ms al superar el umbral ajustado por comando desde la PC.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{LED RGB controlado desde PC:} Implementar un LED RGB en el pin A1 (entrada analógica reservada) para indicar visualmente el nivel de temperatura mediante colores: verde (normal), amarillo (precaución) y rojo (alerta). El color se controlaría mediante comandos seriales enviados desde la interfaz web, reemplazando la función originalmente planteada para el potenciómetro de umbral manual.
...
\begin{itemize}[leftmargin=*]
    \item Se implementaron las \textbf{dos entradas digitales} (DHT11 y pulsador) y la \textbf{entrada analógica} (LDR), verificando su correcta lectura y conversión por el ADC del ATmega328P. El pin A1 quedó reservado para la futura integración del LED RGB documentado.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Manejo de sensores digitales (DHT11, BMP280 vía I2C) y analógicos (LDR), incluyendo técnicas de filtrado y calibración.
