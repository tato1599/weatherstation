...
Previo al montaje físico, se realizaron pruebas de validación lógica en la plataforma \textbf{Wokwi} (\url{https://wokwi.com}), un simulador en línea de circuitos con Arduino que permite ensamblar componentes virtuales, cargar código C++ y observar el comportamiento del sistema sin necesidad de hardware físico.

Wokwi fue seleccionada por las siguientes ventajas:
\begin{itemize}[leftmargin=*]
    \item Soporte nativo para Arduino UNO, sensores DHT22/BMP280 y periféricos comunes.
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}
    \textit{Espacio reservado para captura de pantalla del simulador Wokwi.}\\
    Mostrar el circuito virtual con sensores conectados y consola serial con JSON.
    \vspace{3cm}}}
    \caption{Simulación del circuito en Wokwi con lecturas virtuales.}
    \label{fig:simulacion-wokwi}
\end{figure}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Lectura de sensores:} Se validó que el DHT11 (simulado como DHT22 en Wokwi) y el BMP280 respondieran correctamente a las llamadas de biblioteca y que los valores generados estuvieran dentro de rangos físicos coherentes.
